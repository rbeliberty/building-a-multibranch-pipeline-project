...
dblab clone create --username "${CHANGE_AUTHOR}" --password "${CHANGE_AUTHOR}" --id "${CLONE_ID}" --snapshot-id "${SNAPSHOT_ID}" | jq -r --arg CLONE_ID "${CLONE_ID}" 'select (.id | contains($CLONE_ID)) | .db.port'
